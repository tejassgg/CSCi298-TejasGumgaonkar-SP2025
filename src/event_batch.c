...
// int uv__batch_add_event_internal(uv_loop_t *loop, void *event, size_t event_size) {
//   uv_batch_t *batch_system;
//   uv_batch_event_t *new_event;

//   if (loop == NULL || loop->batch_system == NULL || event == NULL || event_size == 0)
//       return UV_EINVAL;

//   batch_system = (uv_batch_t *)loop->batch_system;

//   new_event = (uv_batch_event_t *)malloc(sizeof(uv_batch_event_t) + event_size);
//   if (new_event == NULL)
//       return UV_ENOMEM;

//   new_event->type = UV_BATCH_EVENT_TYPE_UNKNOWN;
//   new_event->priority = UV_BATCH_PRIORITY_NORMAL;
//   new_event->status = UV_BATCH_STATUS_PENDING;
//   new_event->data = (void *)((char *)new_event + sizeof(uv_batch_event_t));
//   new_event->data_size = event_size;
//   new_event->timestamp = uv__batch_hrtime();
//   new_event->callback = NULL;
//   new_event->next = NULL;

//   memcpy(new_event->data, event, event_size);

//   if (batch_system->queue_head == NULL) {
//       batch_system->queue_head = new_event;
//       batch_system->queue_tail = new_event;
//   } else {
//       batch_system->queue_tail->next = new_event;
//       batch_system->queue_tail = new_event;
//   }
...
// /* Add an event to the batch system */
// int uv__batch_add_event_internal(uv_loop_t *loop, uv_batch_event_type_t type, uv_batch_priority_t priority, void *data, size_t data_size, void (*callback)(void *))
// {
//   uv_batch_t *batch;
//   uv_batch_event_t *event;
//   int start_timer = 0;

//   if (!loop || !loop->batch_system || !data)
//   {
//     return UV_EINVAL;
//   }
...
//   /* Create new event */
//   event = (uv_batch_event_t *)malloc(sizeof(uv_batch_event_t));
//   if (!event)
//   {
//     return UV_ENOMEM;
//   }
...
//   event->status = UV_BATCH_STATUS_PENDING;
//   event->data_size = data_size;
//   event->timestamp = uv__batch_hrtime();
...
//   if (batch->queue_tail == NULL)
//   {
//     batch->queue_head = event;
//     batch->queue_tail = event;
//     start_timer = 1;
//   }
//   else if (priority == UV_BATCH_PRIORITY_HIGH)
//   {
//     event->next = batch->queue_head;
//     batch->queue_head = event;
//   }
//   else
//   {
//     batch->queue_tail->next = event;
//     batch->queue_tail = event;
//   }
...
//   /* Start timeout timer if first event */
//   if (start_timer)
//   {
//     uv_timer_start(&batch->timeout_timer, uv__batch_timer_cb,
//                    UV_BATCH_TIMEOUT_MS, 0);
//   }

//   /* Process batch if full */
//   if (batch->current_size >= UV_BATCH_MAX_SIZE)
//   {
//     uv__batch_process(batch);
//   }
//   else
//   {
//     /* Signal Windows event to potentially wake up IOCP */
//     uv__batch_windows_signal(batch);
//   }
...
/* Enable batching on an event loop */
// int uv_batch_enable(uv_loop_t *loop)
// {
...
// /* Disable batching on an event loop */
// int uv_batch_disable(uv_loop_t *loop)
// {
//   if (!loop)
//   {
//     return UV_EINVAL;
